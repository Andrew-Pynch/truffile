...
Usage:
    ...
    python eval/arxiv_eval.py --tag alphaxiv --provider alphaxiv
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--provider", choices=["v1", "alphaxiv"], default="v1",
        help="Which backend to eval (default: v1)",
    )
    parser.add_argument(
        "--skip-qa", action="store_true",
        help="Skip AlphaXiv Q&A eval (only relevant with --provider alphaxiv)",
    )
...
def print_markdown_report(results: dict) -> None:
    ...
    provider = results.get("provider", "v1")
    duration = results.get("duration_s", 0)
    print(f"\n{'=' * 60}")
    print(f"  Arxiv Eval Results: {tag}  (provider: {provider})")
    ...
    # --- Q&A ---
    if "qa" in results:
        agg = results["qa"]["aggregate"]
        print("### AlphaXiv Q&A\n")
        print("| Metric                    | Value  |")
        print("|---------------------------|--------|")
        print(f"| Success Rate              | {_fmt(agg.get('success_rate'), is_pct=True):>6} |")
        print(f"| Mean Answer Length         | {int(agg.get('mean_answer_length', 0)):>6} |")
        print(f"| Mean Latency              | {_fmt(agg.get('mean_latency_s')):>5}s |")
        print(f"| Total Questions            | {agg.get('total_questions', 0):>6} |")
        print(f"| Total Errors              | {agg.get('total_errors', 0):>6} |")
        print()

        per_q = results["qa"].get("per_question", [])
        if per_q:
            print("| Paper ID      | Question                           | Len   | Time  |")
            print("|---------------|------------------------------------|-------|-------|")
            for q in per_q:
                pid = q.get("paper_id", "?")[:13]
                question = q.get("question", "?")[:36]
                if q.get("status") == "success":
                    print(
                        f"| {pid:<13} | {question:<34} "
                        f"| {q.get('answer_length', 0):>5} "
                        f"| {_fmt(q.get('latency_s')):>5} |"
                    )
                else:
                    err = q.get("error", "unknown")[:30]
                    print(f"| {pid:<13} | {question:<34} | ERROR: {err:<12} |")
            print()
...
async def main() -> None:
    ...
    from arxiv_runners import RateLimiter
    from arxiv_test_set import BG_WORKER_INTERESTS, SEARCH_QUERIES, TEST_PAPERS

    provider = args.provider

    # Validate alphaxiv availability early
    if provider == "alphaxiv":
        from arxiv_config import is_alphaxiv_configured
        if not is_alphaxiv_configured():
            print(
                "ERROR: --provider alphaxiv requires AlphaXiv credentials.\n"
                "Either authenticate via Claude Code's /mcp flow or set ALPHAXIV_ACCESS_TOKEN.",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # AlphaXiv doesn't need rate limiting as aggressively as the v1 arXiv API
    rate_limiter = RateLimiter(min_delay=1.0 if provider == "alphaxiv" else 3.5)

    # Set up isolated storage via temp dirs (needed for v1 extraction)
    ...
    try:
        if provider == "v1":
            from arxiv_runners import run_bg_worker_eval, run_extraction_eval, run_search_eval

            # --- Search eval ---
            if not args.skip_search:
                logger.info("Starting v1 search eval (%d papers, %d queries)...", len(papers), len(SEARCH_QUERIES))
                results["search"] = await run_search_eval(papers, SEARCH_QUERIES, rate_limiter)

            # --- Extraction eval ---
            if not args.skip_download:
                logger.info("Starting v1 extraction eval (%d papers)...", len(papers))
                results["extraction"] = await run_extraction_eval(papers, rate_limiter)

            # --- Background worker eval ---
            if not args.skip_bg:
                logger.info("Starting v1 background worker eval...")
                results["bg_worker"] = run_bg_worker_eval(
                    BG_WORKER_INTERESTS, rate_limiter, llm_judge=args.llm_judge
                )

        elif provider == "alphaxiv":
            from arxiv_runners import (
                run_alphaxiv_extraction_eval,
                run_alphaxiv_qa_eval,
                run_alphaxiv_search_eval,
            )

            # --- AlphaXiv search eval ---
            if not args.skip_search:
                logger.info("Starting alphaxiv search eval (%d papers, %d queries)...", len(papers), len(SEARCH_QUERIES))
                results["search"] = await run_alphaxiv_search_eval(papers, SEARCH_QUERIES, rate_limiter)

            # --- AlphaXiv extraction eval ---
            if not args.skip_download:
                logger.info("Starting alphaxiv extraction eval (%d papers)...", len(papers))
                results["extraction"] = await run_alphaxiv_extraction_eval(papers, rate_limiter)

            # --- AlphaXiv Q&A eval ---
            if not args.skip_qa:
                logger.info("Starting alphaxiv Q&A eval...")
                results["qa"] = await run_alphaxiv_qa_eval(papers, rate_limiter)
